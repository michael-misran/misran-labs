...
const SESSIONS = [
  ...
  { id:'014', date:'2026-03-14', title:'EXP-005 Micro-SaaS Generator', summary:'Une phrase → landing page SaaS complète rendue en iframe', status:'DONE', duration:'45min', tags:['iframe','Streaming','HTML'], what:'Création de MicroSaaSGenerator.jsx : textarea + chips → streaming du code HTML d\'une landing page complète → rendu dans un iframe via toggle CODE/PREVIEW. 4 produits hardcodés (TaskFlow violet, BudgetAI vert, FeedbackLoop orange, LaunchKit bleu fallback). Style SaaS moderne clair pour contraster avec Misran Labs dark. Fix chips post-déploiement.', decision:'Style SaaS clair (fond blanc, Inter) plutôt que Misran Labs dark — le contraste dark/light dans l\'iframe renforce l\'illusion de génération externe. Simulation streaming zéro appel API réel.', prompt:'Crée MicroSaaSGenerator.jsx layout deux colonnes. PAGES_DATA 4 entrées { meta, code }. setInterval 15ms +8 chars. Toggle view code/preview. iframe srcdoc={currentData.code}.' },
  { id:'015', date:'2026-03-14', title:'Portfolio prêt recruteurs', summary:'HomePage identité, /process convaincante, bilan état portfolio', status:'DONE', duration:'1h', tags:['HomePage','Process','Stratégie'], what:'Trois livrables : (1) HomePage — ajout nom "MICHAEL MISRAN — PRODUCT DESIGNER × AI", h1 reécrit, description concrète avec outils nommés. (2) /process — chaque étape enrichie d\'un exemple réel en mono : brief verbatim, prompt de fix, commande git, structure WHAT/DECISION/PROMPT. (3) Réflexions stratégiques documentées : refonte da Vinci écartée, i18n différée, EXP-004 maintenu en pause.', decision:'La page /process affirmait sans prouver — pattern à éviter. Chaque étape du workflow doit avoir un exemple conclet visible. Un recruteur product doit comprendre comment tu penses, pas juste ce que tu fais.', prompt:'Dans ProcessPage.jsx : enrichir chaque STEP avec un champ example contenant un extrait réel de prompt ou de commande. Afficher l\'example en bloc mono avec border-left teal sous les ToolBadges.' },
  { id:'016', date:'2026-03-14', title:'Migration V2 — Shell App', summary:'Portfolio site web → interface applicative Jarvis', status:'DONE', duration:'~2h', tags:['Shell','React','Widgets','Migration'], what:'Reconstruction complète du projet en V2 : remplacement du site multi-pages par un shell applicatif type Jarvis. Topbar live (clock + date + statut ONLINE), Sidebar collapsible 260px/52px, 4 widgets système (horloge, météo Open-Meteo, uptime sessionStorage, module actif), ModuleZone avec fadeIn 0.2s, Statusbar. Navigation state-based via activeModule + navigateTo(). Grid CSS 48px/1fr/32px, overflow hidden full viewport.', decision:'Shell applicatif plutôt que site web classique — différenciation maximale vs un portfolio standard. Chaque recruteur voit un outil, pas une vitrine. Démontre la capacité à concevoir et exécuter une architecture UI complexe sans framework UI.', prompt:'Tu vas reconstruire entièrement le projet Misran Labs en V2. Architecture : shell applicatif de type interface Jarvis. 8 étapes : Shell.jsx, Topbar, Sidebar collapsible, ModuleZone, Statusbar, 4 widgets, HomeModule refait, intégration App.jsx.' },
  { id:'017', date:'2026-03-14', title:'API Cost Monitor', summary:'Dashboard monitoring tokens · coûts · alertes budget', status:'DONE', duration:'~30min', tags:['API','Budget','Tokens','Anthropic'], what:'Création de ApiMonitorModule.jsx : PRICING map 3 modèles (sonnet-4 $3/$15, haiku-4 $0.25/$1.25, opus-4 $15/$75), 5 MOCK_CALLS simulés, calcul coût par appel, stats agrégées (session cost, total tokens, appels, coût moyen). BudgetBar colorée (vert/orange/rouge) selon ratio totalCost/BUDGET_ALERT. CallsTable grid 6 colonnes avec coloration par modèle. ModelBreakdown 3 cards avec proportion bar.', decision:'BUDGET_ALERT = $0.10 — seuil bas intentionnel pour démontrer la logique d\'alerte visuellement lors des démos. Données simulées prêtes à être remplacées par de vrais appels API Anthropic usage endpoint.', prompt:'Crée src/modules/ApiMonitorModule.jsx. PRICING 3 modèles. 5 MOCK_CALLS. calcCost(). BUDGET_ALERT=0.10. StatCard×4, BudgetBar, CallsTable, ModelBreakdown. Intégrer dans Sidebar, ModuleZone, HomeModule.' },
  { id:'018', date:'2026-03-14', title:'Analyse coûts réels', summary:'Facturation Anthropic · Human vs agents · ROI du projet', status:'DONE', duration:'~20min', tags:['Facturation','Tokens','Agents','ROI'], what:'Analyse des coûts réels Claude Code sur le projet Misran Labs : dépassement du quota hebdomadaire 5h, estimation surcoût extra usage, comparaison Human-directed (Claude Code) vs agents autonomes (sous-agents parallèles). Tableau synthèse ROI : coût total projet vs valeur portfolio produite.', decision:'Human-directed reste le mode optimal pour ce projet — contrôle fin sur chaque décision de design/archi, pas de dérive autonome. Les agents parallèles seraient pertinents pour des tâches de génération en masse (ex: 50 session logs d\'un coup).', prompt:'N/A — session d\'analyse et réflexion stratégique, pas de génération de code.' },
]
